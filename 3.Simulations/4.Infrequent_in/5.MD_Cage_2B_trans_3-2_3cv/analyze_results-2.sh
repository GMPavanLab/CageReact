...
rm results01.dat; for i in `seq 0 50`; do awk '{if($5<43 && $7> 0.76){exit}if($2<43 && $4 > 0.76){print $1*$NF; exit;}}' SIM_${i}/COLVAR >> results01.dat; done
rm results02.dat; for i in `seq 0 50`; do awk '{if($2<43 && $4 > 0.76){exit}if($5<43 && $7 > 0.76){print $1*$NF; exit;}}' SIM_${i}/COLVAR >> results02.dat; done
