...
rm results01.dat; for i in `seq 0 50`; do awk '{if($5<40 && $6< 9.5){exit}if($2<40 && $3 < 9.5){print $1*$NF; exit;}}' SIM_${i}/COLVAR >> results01.dat; done
rm results02.dat; for i in `seq 0 50`; do awk '{if($2<40 && $3 < 9.5){exit}if($5<40 && $6 < 9.5 ){print $1*$NF; exit;}}' SIM_${i}/COLVAR >> results02.dat; done
