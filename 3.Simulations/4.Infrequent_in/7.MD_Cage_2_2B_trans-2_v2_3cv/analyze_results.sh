...
rm results1.dat; for i in `seq 0 50`; do awk '{if(($2<35 && $3 > 0.9) || ($4<35 && $5 > 0.9)){print $1*$NF; exit;}}' SIM_${i}/COLVAR >> results1.dat; done
rm results01.dat; for i in `seq 0 50`; do awk '{if(NR!=1 && ($2<3 || $4<3)){exit}else if(($2<40 && $2>20 && $3>0.8 && $3< 1.0) || ($4<40 && $4>20 && $5>0.8 &&    $5< 1.0)){print $1*$NF; exit;}}' SIM_${i}/COLVAR >> results01.dat; done
